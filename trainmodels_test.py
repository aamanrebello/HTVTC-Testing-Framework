from trainmodels import crossValidationFunctionGenerator, evaluationFunctionGenerator, applyTrainingSamplesBudget, applyTrainingFeaturesBudget
...
mockDescriptionDict = {
            'trainval_features': [[1,2], [2,2], [0,1]],
            'trainval_labels': [0.1, 0.4, 0.0],
            'index_generator': [([0,1],[2]), ([0,2],[1]), ([1,2],[3])],
            'no_splits': 3,
            'test_features': [],
            'test_labels': []
        }
...
#Test the crossValidationFunctionGenerator that is built on top of evaluationFunctionGenerator
class TestCrossValidationFunctionGenerator(unittest.TestCase):

    def test_incorrect_task(self):
        #Perform test for ridge regression
        test = False
        try:
            result = crossValidationFunctionGenerator(descriptionDict = mockDescriptionDict, algorithm = 'ridgeregression', task = 'classification')
        except ValueError as inst:
            self.assertEqual(inst.args[0], 'The algorithm specified is not recognised.')
            test = True
        self.assertTrue(test)
        ...
        test = False
        try:
            result = crossValidationFunctionGenerator(descriptionDict = mockDescriptionDict, task = 'regression')
        except ValueError as inst:
            self.assertEqual(inst.args[0], 'The algorithm specified is not recognised.')
            test = True
        self.assertTrue(test)
        ...
        test = False
        try:
            result = crossValidationFunctionGenerator(descriptionDict = mockDescriptionDict, algorithm = 'svm-polynomial', task = '***')
        except ValueError as inst:
            self.assertEqual(inst.args[0], 'The algorithm specified is not recognised.')
            test = True
        self.assertTrue(test)
        ...
        test = False
        try:
            result = crossValidationFunctionGenerator(descriptionDict = mockDescriptionDict, algorithm = 'knn-regression', task = 'rgression')
        except ValueError as inst:
            self.assertEqual(inst.args[0], 'The algorithm specified is not recognised.')
            test = True
        self.assertTrue(test)
        ...
        test = False
        try:
            result = crossValidationFunctionGenerator(descriptionDict = mockDescriptionDict, algorithm = 'knn-classification', task = 'clasification')
        except ValueError as inst:
            self.assertEqual(inst.args[0], 'The algorithm specified is not recognised.')
            test = True
        self.assertTrue(test)
        ...
        test = False
        try:
            result = crossValidationFunctionGenerator(descriptionDict = mockDescriptionDict, algorithm = 'random-forest', task = 'regression')
        except ValueError as inst:
            self.assertEqual(inst.args[0], 'The algorithm specified is not recognised.')
            test = True
        self.assertTrue(test)
    ...
    def test_unknown_algorithm(self):
        ...
        test = False
        try:
            result = crossValidationFunctionGenerator(descriptionDict = mockDescriptionDict, algorithm = 'unknown', task = 'classification')
        except ValueError as inst:
            self.assertEqual(inst.args[0], 'The algorithm specified is not recognised.')
            test = True
        self.assertTrue(test)
    ...
    def test_ridge_regression(self):
        ...
        func = crossValidationFunctionGenerator(data_split, algorithm='ridge-regression', task=task)
        self.assertIsNotNone(func)
    ...
    def test_KNN_regression(self):
        ...
        task = 'regression'
        data = loadData(source='sklearn', identifier='california_housing', task=task)
        data_split = trainTestSplit(data, method='cross_validation', validation_proportion=0.3)
    ...
    def test_no_budget_fraction(self):
        ...
        test = False
        try:
            func = crossValidationFunctionGenerator(data_split, algorithm='ridge-regression', task=task, budget_type='samples')
        except ValueError as inst:
            self.assertEqual(inst.args[0], 'A budget fraction has not been provided.')
            test = True
        self.assertTrue(test)
