...
BASE_PATH = 'saved-cross-validation-arrays/svm-polynomial/'
...
if load_tensor:
    ...
    nobudgetfunc = crossValidationFunctionGenerator(data_split, algorithm='svm-polynomial', task=task)
    ...
    tensor, _ = generateIncompleteErrorTensor(nobudgetfunc, ranges_dict, 1.0, metric=classificationmetrics.hingeLoss, evaluation_mode='raw-score', eval_trials=1)
...
budgetfunc = crossValidationFunctionGenerator(data_split, algorithm='svm-polynomial', task=task, budget_type=budget_type, budget_fraction=budget_fraction)

known_fraction = 0.25
incomplete_tensor, known_indices = generateIncompleteErrorTensor(budgetfunc, ranges_dict, known_fraction, metric=classificationmetrics.hingeLoss, evaluation_mode='raw-score', eval_trials=1)
